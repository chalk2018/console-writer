...
declare const globalThis: { console: globalThis.Console, __console: globalThis.Console };
// console有效属性
...
export interface Option {
    ...
    // 手动配置log4js
    configLog4js?: (log4js, setLogger: (logger) => any, rewriteConsole: () => any) => any,
    // 其他配置
...
export class ConsoleBuilder extends Map {
    ...
    // hooks
    onLog(callback: (console, logger) => void | undefined | ((console, logger) => any)) {
        this.set('on_log', callback);
    }

    onInfo(callback: (console, logger) => void | undefined | ((console, logger) => any)) {
        this.set('on_info', callback);
    }

    onError(callback: (console, logger) => void | undefined | ((console, logger) => any)) {
        this.set('on_error', callback);
    }

    onWarn(callback: (console, logger) => void | undefined | ((console, logger) => any)) {
        this.set('on_warn', callback);
    }

    onDebug(callback: (console, logger) => void | undefined | ((console, logger) => any)) {
        this.set('on_debug', callback);
    }

    hookingStart(logType: LogType) {
    ...
    hookingEnd(logType: LogType, hooksMap: { [hook: string]: (console, logger) => any }) {
        const hook = this.get('on_' + logType);
        if (hook) {
    ...
    configResolver(fileName?: string) {
        try {
    ...
    output(logType: LogType, msgs: Array<string>) {
        const _this = this;
    ...
    logWriter(logType: LogType, context?: any) {
        const _this = this;
        ...
        function logWrite(...args) {
            ...
            const arg1 = args[0];
            if (typeof arg1 === 'string' && ['%o', '%O', '%d', '%i', '%s', '%f'].some((fmt: string) => arg1.indexOf(fmt) >= 0)) {
                // 模板输出格式
                let i = 1;
